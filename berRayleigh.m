...
for ii = 1:length(Eb_N0_dB)
   ...
   %without equalizer
   ipHatwe = real(y)>0;
   nErrwe(ii) = size(find([ip- ipHatwe]),2);
   % counting the errors
...
semilogy(Eb_N0_dB,simBerwe,'rv-','LineWidth',2);
axis([-3 35 10^-5 1.5])
grid on
legend('AWGN-Theory','Rayleigh-Theory', 'Rayleigh-Simulation', 'Rayleigh-Sim. without Equalization');
xlabel('Eb/No, dB');
